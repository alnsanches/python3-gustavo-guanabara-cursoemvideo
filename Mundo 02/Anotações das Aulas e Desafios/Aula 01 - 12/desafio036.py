...
valorimovel = float(input('Qual é o valor da casa que você deseja comprar? R$ '))
salario = float(input('Quanto você ganha por mês? R$ '))
anos = int(input('Em quantos anos você pretende pagar? '))
meses = anos * 12
prestacoes = valorimovel / meses
minimo = salario * 30 / 100
print('Então você deseja comprar uma casa no valor R$ {:.2f}'.format(valorimovel), end=' ')
